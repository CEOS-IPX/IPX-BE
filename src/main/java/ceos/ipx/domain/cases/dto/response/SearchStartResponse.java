 ...
 * 프론트는 이 응답의 caseId로 폴링해서 진행 상태 조회
 * 완료되면 caseId로 GET /prior-arts 호출
 */
@Schema(description = "선행기술 탐색 시작 응답")
public record SearchStartResponse(

        @Schema(description = "생성된(또는 재사용된) 사건 ID (진행 상태 폴링에 사용)", example = "1")
        Long caseId,

        @Schema(description = "검색 시작 상태 표시 (항상 \"started\")", example = "started")
        String status
) {
    public static SearchStartResponse of(Long caseId) {
        return new SearchStartResponse(caseId, "started");
    }
}
